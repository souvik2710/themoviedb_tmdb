...
import 'package:themoviedb_tmdb/authencation/sign_in.dart';
import 'package:themoviedb_tmdb/common_widgets/text_field.dart';
import 'package:themoviedb_tmdb/main_page/bottom_navigation_bar.dart';
...
class SignUpPage extends HookConsumerWidget {
  ...
  Widget build(BuildContext context,WidgetRef ref) {
    final eNameController = useTextEditingController();
    final eEmailController = useTextEditingController();
    final ePasswordController = useTextEditingController();
    final ePasswordConfirmController = useTextEditingController();
    return Scaffold(
              ...
              Align(
                ...
                child: Column(
                  children: [
                    ...
                    const Text('Sign Up',style: TextStyle(fontSize: 20,fontWeight: FontWeight.w500),),
                    ...
                    SizedBox(height: MediaQuery.of(context).size.height*0.032,),
                    NewCommonMainTextField(labelTextPass: 'Your Name',textEditingController: eNameController,
                      preFixIconPass: const Icon(Icons.account_circle),
                      onChanged: (value){
                    },
                      isEnable: true,
                      inputFormattersPass: [
                    ...
                    SizedBox(height:MediaQuery.of(context).size.height*0.032,),
                    NewCommonMainTextField(labelTextPass: 'Confirm Password',textEditingController: ePasswordConfirmController,
                      preFixIconPass: const Icon(Icons.lock),
                      isObscureText: true,
                      isTypePassword: true,
                      inputFormattersPass: [
                    ...
                    SizedBox(height:MediaQuery.of(context).size.height*0.032,),
                    const Text('By signing up, you accept Privacy Policy & Terms of Service',style: TextStyle(fontSize: 12,color: Colors.grey),),
                    ...
                    ElevatedButton(
                        ...
                        onPressed: ()async{
                         await  ref.read(movieViewModel).signUpFirebase(eEmailController.text, ePasswordController.text,eNameController.text);
                         await ref.read(movieViewModel).updateUserCloudF( bookmarkList: []);
                         await Navigator.of(context).push(MaterialPageRoute(builder: (context)=>BottomNav()));
                         // await ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text('Signed Up Successfully')));
                    ...
                    RichText(text:  TextSpan(
                      ...
                      children: [
                        WidgetSpan(child: InkWell(
                          ...
                          child: const Text('Sign in',style: TextStyle(color: ConstantColors.highlightedColor),),
                        )),
                      ],
                    ),),
